...
trait ConnectionIOOps:
  ...
  extension [T](connection: ConnectionIO[T])
    ...
    /** Runs the current ConnectionIO, then runs the parameter, keeping its result. The result of the first action is
      * ignored. If the source fails, the other action won't run. Evaluation of the parameter is done lazily, making
      * this suitable for recursion.
      */
