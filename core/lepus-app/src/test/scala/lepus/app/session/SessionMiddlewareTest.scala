...
object SessionMiddlewareTest extends Specification:
  ...
  val sessionRoutes: SessionRoutes[IO, Option[Session]] = SessionRoutes.of[IO, Option[Session]] {
    case GET -> Root / "session" / "init"   as _       => Ok("Session init").map(resp => ContextResponse(Some(Session("Init")), resp))
    case GET -> Root / "session" / "reset"  as _       => Ok("Session reset").map(resp => ContextResponse(None, resp))
    case GET -> Root / "session" / "update" as session => Ok("Session update").map(resp => ContextResponse(session.map(_.copy(name = "Updated")), resp))
  }
  ...
  "Testing the SessionMiddleware" should {
    ...
    "With the identifier in place, the value stored in storage can be updated." in {
      val result = for
        ...
        session <- storage.get(SessionIdentifier(id.getOrElse("")))
      yield id.isDefined and old.isDefined and old.map(_.name).contains("Init") and session.isDefined and session.map(_.name).contains("Updated") and old != session
  ...
  private def selectIdFromResponseHeader(response: Option[Response[IO]], key: String = "LEPUS_SESSION_TEST"): Option[String] =
    response.flatMap(_.headers.get(CIString("Set-Cookie"))).flatMap(_.head.value.split(';').headOption.map(_.replace(s"$key=", "")))
