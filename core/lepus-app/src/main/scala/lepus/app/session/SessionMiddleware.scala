...
private[lepus] object SessionMiddleware extends SessionConfigReader:
  ...
  def fromConfig[F[_]: Monad: Clock, A](
    ...
    val deleteCookie = generateDeleteCookie(sessionIdentifier, sessionDomain, sessionPath, sessionSameSite, sessionSecure, sessionHttpOnly)
    val expiration: ExpirationManagement[F] = sessionExpirationType match
      ...
      case "Dynamic" => ExpirationManagement.Dynamic[F](
        httpDate => Monad[F].pure(ExpirationManagement.Static(httpDate.epochSecond.some, httpDate.some))
      )
      case unknown => throw new IllegalArgumentException(s"$unknown did not match any of the ExpirationManagement. The value of ExpirationManagement must be Static, or Dynamic.")
