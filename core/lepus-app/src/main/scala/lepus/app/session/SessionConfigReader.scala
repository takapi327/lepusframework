...
private[lepus] trait SessionConfigReader:
  ...
  protected lazy val sessionIdentifier: String = config.get[Option[String]](SESSION_IDENTIFIER).getOrElse("LEPUS_SESSION")
  ...
  protected lazy val sessionSameSite: SameSite = config.get[Option[String]](SESSION_SAME_SITE)
    .fold(SameSite.Lax)(str => str match
      case "Lax"    => SameSite.Lax
      case "Strict" => SameSite.Strict
      case "None"   => SameSite.None
      case unknown  => throw new IllegalArgumentException(s"$unknown did not match any of the SameSites. The value of SameSite must be Lax, Strict, or None.")
    )

  /** Type of object that controls the cookie expiration date. Default is Static */
  protected lazy val sessionExpirationType: String = config.get[Option[String]](SESSION_EXPIRATION_TYPE).getOrElse("Static")
  ...
  protected lazy val sessionExpirationExpires: Option[HttpDate] =
    config.get[Option[Long]](SESSION_EXPIRATION_EXPIRES)
      .map(long => HttpDate.fromEpochSecond(long) match
