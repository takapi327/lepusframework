...
object Validator {
  ...
  case class Pattern(value: String) extends Validator {
    def apply(t: String): Option[DecodeResult.Failure] = {
      if (t.matches(value)) None
      else Some(DecodeResult.InvalidValue(s"$t did not match the regular expression in $value", None))
    }
  }
  ...
  case class Range(min: Int, max: Int) extends Validator {
    ...
    def apply(t: String): Option[DecodeResult.Failure] = {
      try
        if (min <= t.toInt && t.toInt <= max) None
